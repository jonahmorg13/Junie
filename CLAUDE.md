...
- **Custom terminal-y primitives, no Material 3.** Typography splits three ways: Noto Serif (`HeaderFont`) for headers and page titles, Noto Sans (`BodyFont`) for running body text, JetBrains Mono (`MonoFont`) reserved for code blocks, inline code, and button labels. All three font families live in `ui/theme/Theme.kt` and are hard-pinned — there's no user-facing font picker. New screens reuse the `ui/terminal/` primitives on top of a rounded `TermBox` panel against the active palette. The chat composer's input intentionally has `showBorder = false`; every other input keeps its rounded border. (The "terminal" naming is historical — only code/labels are monospace now.)
